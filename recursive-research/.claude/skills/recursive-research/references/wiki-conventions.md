...
Standards for the research wiki in `projects/<slug>/wiki/`. The wiki is a set of interlinked markdown files that grow with each iteration. The synthesis-agent authors all wiki content; research-lead reads it for scoring.
...
- All filenames: lowercase-kebab-case (e.g., `mcp-server-go-implementation.md`)
...
| source-2 | <URL> | <domain> | <what it supports> | Blocked (domain not whitelisted) |
...
## Key Facts
<Bullet list of verified facts with inline citations>
...
- [[another-subtopic]] -- <relationship>
```

## Marker Syntax

Use these markers exactly as written -- research-lead counts them for scoring.
...
| `[source-N]` | Inline citation | `The MCP spec uses JSON-RPC 2.0 [source-1][source-2].` |
...
- Use `[[subtopic-name|display text]]` when the filename is too long

## Source Quality Tiers

When adding sources to the bibliography, note the tier:

| Tier | Description | Trust Level |
|------|-------------|-------------|
| Primary | Official docs, spec text, authors' own writing | Highest |
| Secondary | Reputable technical blogs, HN, Stack Overflow | High |
| Tertiary | General web articles, aggregators | Medium -- cross-verify |
| Snippet-only | WebSearch result snippet (page not fetched) | Low -- note as such |

## Completeness Counting

For scoring purposes, a subtopic page is considered "gap-free" (contributing to completeness) when:
- It has zero `[GAP]` markers
- It has at least 3 unique sources cited
- It has at least one paragraph of substantive content (not just markers)

The denominator for completeness is the number of sub-topics listed in `program.md`.
